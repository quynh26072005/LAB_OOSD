...
namespace QuanLyThuVien.Services
{
    public class MuonTraService
    {
                ...
                try
                {
                    ...
                    foreach (string maSach in unique)
                    {
                        ...
                        using (SqlCommand update = new SqlCommand(@"UPDATE DauSach SET SoLuongHienCo=SoLuongHienCo-1 WHERE MaDauSach=@Ma AND SoLuongHienCo>0", cn, tx))
                        {
                            ...
                            if (update.ExecuteNonQuery() != 1) throw new InvalidOperationException("Không thể cập nhật tồn kho.");
                        }
                        i++;
                    }
                    tx.Commit();
        ...
        {
            return Db.Query(@"SELECT ct.MaChiTiet, pm.MaPhieuMuon, pm.MaDocGia, ct.MaDauSach, s.TenSach, pm.NgayMuon, pm.NgayHenTra FROM PhieuMuon pm JOIN ChiTietPhieuMuon ct ON ct.MaPhieuMuon=pm.MaPhieuMuon JOIN DauSach s ON s.MaDauSach=ct.MaDauSach WHERE pm.MaDocGia=@Ma AND ct.NgayTraThucTe IS NULL ORDER BY pm.NgayHenTra, s.TenSach", new SqlParameter("@Ma", maDocGia));
        }
        ...
        public KetQuaXuLy TraSach(string maChiTiet, string maNhanVien, DateTime ngayTra, string tinhTrang, decimal phiPhat)
        {
            ...
            using (SqlTransaction tx = cn.BeginTransaction(IsolationLevel.Serializable))
            {
                try
                {
                    ...
                    if (canPhat && phiPhat <= 0) return KetQuaXuLy.Loi("Trường hợp trả trễ/mất/hư hỏng phải nhập phí phạt cụ thể lớn hơn 0.");
